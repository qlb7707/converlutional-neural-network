...
class LogisticRegression(object):
    def __init__(self,input,n_in,n_out):
        self.W = theano.shared(value = numpy.zeros((n_in\
        ...
        self.p_y_given_x = T.nnet.softmax(T.dot(input,self.W)\
                +self.b)
        '''
        tensor.argmax returns the index of maxium in the given axis
        '''
        self.y_pred = T.argmax(self.p_y_given_x,axis=1)
        self.params = [self.W,self.b]

    def negative_log_likelihood(self,y):
        return -T.mean(T.log(self.p_y_given_x)[T.arange(\
                y.shape[0]),y])

    def errors(self,y):
        if y.ndim != self.y_pred.ndim:
            raise TypeError('y should have the same shape as\
                     self.y_pred.type',('y',target.type,'y_pred'\
                     ,self.y_pred.type))
...
def sgd_optimization_mnist(learning_rate = 0.13,n_epochs=1000,\
    ...
    print(('Optimization complete with best validation score of %f %% with\
             test performance %f %%')%(best_validation_loss*100.,test_score*100.))
